...
Исполняемый файл должен быть помещен на две директории глубже рабочего пространства приложения, где расположена директория ```appendix```, по умолчанию: ```${workspace}/build/src``` или ```${workspace}/build/tests```.
