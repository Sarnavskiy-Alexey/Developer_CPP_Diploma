...
#### Запуск
Для запуска проекта необходимо выполнить конфигурацию CMake, далее выполнить сборку и после этого запустить исполняемый файл. Исполняемый файл должен быть помещен в директорию bin рабочего пространства ```{workspace_directory}/bin/```.

Для запуска приложения с тестами необходимо в CMakeLists добавить следующий макрос:
```cmake
add_compile_definition(__TEST_SEARCH_ENGINE__)
```
